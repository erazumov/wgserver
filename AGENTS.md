...
- **xray is the exit, not WireGuard.** All egress for `wg0` clients is achieved by iptables NAT rules that rewrite the destination to a non-loopback local IP (192.0.2.1, RFC 5737 TEST-NET-1) bound to `lo`: `iptables -t nat -A PREROUTING -i wg0 -p tcp -j DNAT --to-destination 192.0.2.1:10808` for TCP, and `iptables -t mangle -A PREROUTING -i wg0 -p udp -j TPROXY --tproxy-mark 0x1/0x1 --on-port 10808 --on-ip 192.0.2.1` for UDP. The OUTPUT chain has the matching daemon-uid rules. xray's inbound is `dokodemo-door` on `127.0.0.1:10808`... no wait, on `192.0.2.1:10808` (see "Why a non-loopback xray IP" below) with `network: tcp,udp` and `followRedirect: true` so it reads the original destination via `SO_ORIGINAL_DST` (TCP) or the TPROXY mark (UDP) and dials the real target. There is no MASQUERADE by default, no policy routing, no second routing table, no `Table = off` trick — all of that was scaffolding for the WG-as-exit topology and is obsolete.
- **Why a non-loopback xray IP.** xray-core's `app/proxyman/inbound/worker.go` has an anti-loopback check: for non-`AnyIP` listeners, it compares `w.hub.Addr().String()` to `dest.NetAddr()` (the `GetOriginalDestination` result) and drops the connection with "loopback connection detected" if they match. For TCP via `REDIRECT` from PREROUTING, `GetOriginalDestination` (via `getsockopt(SO_ORIGINAL_DST, ...)`) reliably returns the pre-NAT destination for the daemon's `OUTPUT`-REDIRECT path, but for peer traffic the same path can resolve to the same `IP:port` as the listener when the read bytes are misinterpreted (the `unsafe.Pointer(&addr.Addr.Flowinfo)` shortcut in xray v26.x returns 0.0.0.0 for IPv4 sockets, and the `Destination` builder may then fall back to `127.0.0.1`). With a non-loopback IP (`192.0.2.1`) the inequality always holds, the check passes, and the connection is accepted. The IP is reserved (RFC 5737), never routable on the public internet, so there's no conflict risk.
- **MASQUERADE for peers is opt-in.** `WGSERVER_MASQUERADE_PEERS=1` in `deploy.env` (re-)installs `iptables -t nat -A POSTROUTING -s 10.0.1.0/24 ! -d 10.0.1.0/24 -j MASQUERADE`. This is **only** needed for traffic that bypasses xray (ICMP, anything xray-core rejects): with the default design, peer TCP/UDP is proxied through xray which performs its own source-NAT, but ICMP and any non-proxied protocol exit the box as `10.0.1.X` and the public internet can't route back to RFC 1918 addresses. Trade-off: enabling it bypasses xray for those packets (the server's public IP is what the internet sees, not the VLESS exit's IP). Off by default; flip it on if `ping` / `traceroute` from peers matters more than funneling every byte through VLESS.
- **xray must NOT run as the wgserver user.** If it did, its own outbound VLESS connection to the remote would match the `OUTPUT -m owner --uid-owner wgserver` REDIRECT rule, xray would dial itself, and you'd get a silent infinite loop that looks like "xray is up but every connection times out". xray runs as the dedicated `xray` system user with `AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE`: `CAP_NET_ADMIN` for `setsockopt(IP_TRANSPARENT)` (required for TPROXY UDP forging — xray must be able to bind a UDP socket to a non-local address like `1.1.1.1:53` to send the response back from the original destination); `CAP_NET_BIND_SERVICE` to allow that bind to privileged destination ports (<1024) — without it, every DNS query from a peer fails with `fake: socket bind: permission denied` in xray's journal. The systemd unit enforces this.
...
- **xray config is operator-managed, not generated.** `/etc/xray/config.json` is hand-written by the operator (copy-paste from their xray client profile, with the inbound swapped to `dokodemo-door`). `install.sh` validates that it exists, parses as JSON, and the first inbound is `dokodemo-door` on `192.0.2.1:10808` (matches `XRAY_LISTEN_IP` in `deploy/install.sh`). `install.sh` does NOT rewrite the operator's VLESS Reality secrets. To rotate VLESS credentials, edit `/etc/xray/config.json` and `systemctl restart xray` — no wgserver restart needed.
...
- Required on first install: `/etc/xray/config.json` must exist and have a `dokodemo-door` inbound on `192.0.2.1:10808` (see AGENTS.md invariant "xray config is operator-managed"). `install.sh` aborts with a clear error otherwise.
